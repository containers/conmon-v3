use std::{os::fd::OwnedFd, path::{Path, PathBuf}};
...
use nix::{
    NixPath, errno::Errno, fcntl::{AT_FDCWD, OFlag, open}, sys::{
        socket::{
...
impl UnixSocket {
    ...
    pub fn listen(
        ...
        if let Some(path) = path {
            ...
            let fallback;
            let dir = if let Some(parent) = path.parent() && !parent.is_empty() {
                parent
            } else {
        ...
        unlink(&full_path)
            .or_else(|e| {
                if e == nix::Error::ENOENT {
                    ...
                    Err(ConmonError::new(format!("Failed to remove old socket {full_path:?}: {e}"), 1))
                }
            })?;
    ...
    fn bind_relative_to_dir(
        ...
        let addr = if let Some(dfd) = dir_fd {
            ...
            UnixAddr::new(path)
                .or_else(|e| Err(ConmonError::new(format!("Failed to create UnixAddr from {path:?}: {e:?}"), 1)))?
        } else {
            UnixAddr::new(path)
                .or_else(|e| Err(ConmonError::new(format!("Failed to create UnixAddr from {path:?}: {e:?}"), 1)))?
        };
    ...
    fn socket_parent_dir(
        ...
        if base_path.is_empty() {
            return Err(ConmonError::new("Base path for socket cannot be determined", 1));
        }
    ...
    pub fn accept(&self) -> ConmonResult<Option<RemoteSocket>> {
        ...
        match accept(self.fd.as_ref().unwrap().as_raw_fd()) {
            Ok(new_fd) => {
                let remote = RemoteSocket::new(self.socket_type, unsafe { OwnedFd::from_raw_fd(new_fd) });
                Ok(Some(remote))
            }
