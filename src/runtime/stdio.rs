use crate::{
    ...
    logging::plugin::LogPlugin, unix_socket::{RemoteSocket, SocketType, UnixSocket},
};

use nix::{
    errno::Errno, fcntl::OFlag, poll::{PollFd, PollFlags, PollTimeout, poll}, sys::socket::{SockaddrStorage, recvfrom}, unistd::{pipe2, read, write}
};
...
mod tests {
    ...
    fn handle_stdio_calls_write_for_stdout_and_stderr() -> ConmonResult<()> {
        ...
        handle_stdio(&mock, &r_out, &r_err, None, None)?;
        Ok(())
    }
