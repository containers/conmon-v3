...
use nix::sys::{socket::{SockFlag, SockType}, stat::Mode};
...
impl RuntimeSession {
    ...
    pub fn launch(
        ...
        attach_socket.listen(Some(PathBuf::from("attach")),
            SockType::SeqPacket,
    ...
    pub fn write_exit_code(&mut self, api_version: i32) -> ConmonResult<()> {
        ...
        if let Some(fd) = self.sync_pipe_fd.take() {
            if let Some(mainfd_stderr) = &self.mainfd_stderr {
                let err_str = read_pipe(mainfd_stderr)?;
                self.sync_pipe_fd =
    ...
    pub fn run_event_loop(&mut self, log_plugin: &dyn LogPlugin) -> ConmonResult<()> {
        #[allow(clippy::collapsible_if)]
...
mod tests {
    ...
    fn run_event_loop_errors_without_stdio() -> ConmonResult<()> {
        ...
        let err = sess.run_event_loop(&NoopLog).unwrap_err();
        let msg = format!("{err}");
        assert!(
